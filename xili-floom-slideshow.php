 ...
 */
if (!is_admin()) {
	$name_selector = ""; /* used in array callback to subselect image filename */
	/**
	...
	function image_name_filter($image) {
		global $name_selector;
		if (strpos($image->post_name,$name_selector) === false ) {
			return false;
		} else {
			return true;
		}
	...
	function xilifloom_insert_script() { 
		global $post, $xili_settings, $name_selector;
		/* some vars can be set inside each post */
		$children = get_post_meta($post->ID, 'floom_parentID', true); /* images are kept in another post */
		$children = ('' != $children) ? (int)$children : $post->ID ; 
		
		$name_selector = get_post_meta($post->ID, 'floom_subname', true);
		$images = xilifloom_getimages($children);
		
		$title_desc = get_post_meta($post->ID, 'floom_title_desc', true);
		$title_desc = ('' != $title_desc) ? $title_desc : 1 ;
		...
		$amount = ('' != $amount) ? $amount : $xili_settings['amount']; 
		$interval = get_post_meta($post->ID, 'floom_interval', true);
		$interval = ('' != $interval) ? $interval : $xili_settings['interval']; 
		$axis = get_post_meta($post->ID, 'floom_axis', true);
		$axis = ('' != $axis) ? $axis : $xili_settings['axis'] ;
		...
		<script type="text/javascript" charset="utf-8">
			...
			if ($chk($('<?php echo $blinds; ?>'))) {
			<?php /* images src absolute url so slidesBase = '' */ 
			if (false !== $images) {
				if ('' != $name_selector) { /* based on name sub string */
					...
					foreach ($images as $attach_ID => $image)	 {
						 ...
						 switch ($title_desc) {
						 	...
						 	case 2:
						 		...
						 		break;
						 	case 3:
						 		$caption = $image->post_title.'<br/><span class="'.$xili_settings['prefix'].'desc">'.$image->post_content.'</span>';	
						 		break;
						 } 	
