...
# 0.9.0 First public
# Version date 110602 - MS

/** 
 * version 
 */
define('XILIFLOOM_VER','0.9.7');
...
class xili_floom_activate {
	...
	function xili_floom_activate () {
		...
		if ( !is_admin() ) {
			...
			add_action( 'wp_head', array( &$this, 'xilifloom_insert_script' ), 20);
		}
		/** 
	...
	function xilifloom_get_values() {
		...
		$this->xilifloom_name_selector = get_post_meta($post->ID, 'floom_subname', true); /* ever lowercase = slug*/
	...
	function xilifloom_insert_script() { 
		...
		if ( has_filter('xili_floom_get_values') ) {
			...
			$xili_floom_values = $this->xilifloom_get_values();
		}
		...
		if ( has_filter( 'xili_floom_get_images' ) ) {
			$images = apply_filters( 'xili_floom_get_images', $children, $this->xilifloom_name_selector );
		} else {
			
			$images = $this->xilifloom_get_images( $children, $this->xilifloom_name_selector );
		}
		if ( is_singular() ) $this->xili_singular_images = $images;
				?>
		...
		window.addEvent('domready', function(e) {	
			...
			if ((false === $images && $display == 'none') || (count($images) < 1 && $display == 'none')) {
				...
				// Floom or xiliFloom
				theFloom = new <?php echo $this->xili_settings['goldparam']; ?>Floom('<?php echo $floom_divs; ?>',slides, {
	 ...
	 * @since 0.9.6 - via guid because post_name don't contains name of file as before 3.0
	 */ 
	function where_post_name_subselect( $where ){
		global $wpdb ;
		$column = $this->xili_settings['post_column'];
		if ( $column == 'guid' ) {
			$like_str = ( substr($this->xilifloom_name_selector, 0, 1 ) != "%" ) ? "%".$this->xilifloom_name_selector : $this->xilifloom_name_selector  ;
		} else {
			$like_str = $this->xilifloom_name_selector ;
	...
	function xili_floom_active_menu(){
		...
		<div id="xilifloom-settings" class="wrap">
			...
			<h4><a href="http://dev.xiligroup.com/xili-floom-slideshow" title="Plugin page and docs" target="_blank" style="text-decoration:none" ><img style="vertical-align:middle" src="<?php echo plugins_url( 'images/xilifloom-logo-32.png', __FILE__ ) ; ?>" alt="xili-floom logo"/>  xili-floom-slideshow</a> - © <a href="http://dev.xiligroup.com" target="_blank" title="<?php _e('Author'); ?>" >xiligroup.com</a>™ - msc 2009-2011 - v. <?php echo XILIFLOOM_VER; ?></h4>
			</div>
					</div>
			</form>
		</div>
...
function xilifloom_get_values() {
	...
	return $xili_floom_activate->xilifloom_get_values();
}
